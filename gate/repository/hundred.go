...
func (hr *HundredRepository) FindKList(tenKHundred uint) ([]model.Hundred, error) {
	var hundreds []model.Hundred
	err := hr.db.Where("rank = ?", model.Rank1000).Where("hundreds between ? and ?", tenKHundred, tenKHundred+model.Rank10000).Order("hundred desc").Find(&hundreds).Error
	return hundreds, err
}
...
func (hr *HundredRepository) FindHList(kHundred uint) ([]model.Hundred, error) {
	var hundreds []model.Hundred
	err := hr.db.Where("rank = ?", model.Rank1000).Where("hundreds between ? and ?", kHundred, kHundred+model.Rank1000).Order("hundred desc").Find(&hundreds).Error
	return hundreds, err
}
